...
def test_delta_detects_nested_edge():
    delta = {"context": {"pending": [{"edge": {EDGE.id: {"action": 1}}}]}}
    assert ImpactBuilder.delta_mentions_edge(delta, EDGE)


def test_delta_rejects_reverse_edge():
    delta = {"edges": {"x": {
        "itemtype_source": "Computer", "items_id_source": "16",
        "itemtype_impacted": "Computer", "items_id_impacted": "15",
    }}}
    assert not ImpactBuilder.delta_mentions_edge(delta, EDGE)
